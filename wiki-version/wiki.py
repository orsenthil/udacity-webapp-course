...
class WikiPage(Handler):
    ...
    def get(self, pagename):
        ...
        if self.request.query:
            revision = int(self.request.get('v'))
...
class EditPage(Handler):
    ...
    def post(self, *args, **kwargs):
        ...
        if not title:
            title='index';
        page = db.GqlQuery("SELECT * FROM WikiEntry WHERE title=:1 ORDER BY created DESC limit 1;", title).get()
        if not page:
...
class HistoryPage(Handler):
    ...
    def render_history_page(self, **kwargs):
    ...
    def get(self, pagetitle):
        ...
        if not title:
            title='index';
        page_history = db.GqlQuery("SELECT * FROM WikiEntry WHERE title=:1 ORDER BY created DESC;", title).fetch(limit=None)
        revisions = len(page_history)
        self.render_history_page(title=title,page_history=page_history,revisions=revisions)
